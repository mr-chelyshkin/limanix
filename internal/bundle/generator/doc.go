...
//
// Builds use a temporary directory outside embedded resources. All targets are
// built and validated before publication begins. Each archive is replaced
// atomically and the manifest is written last; this is not a transaction across
// all files. Run generation before compiling the embedding application.
//
// A reuse failure is reported through the supplied diagnostic writer and triggers
// rebuilding. A build or publication failure returns an error. Runtime cache
// validation belongs to the parent bundle package.
//
// Read generate.go for ordering, toolchain.go and environment.go for compiler
// selection, source.go and plan.go for expected inputs, executable.go and
// metadata.go for validation, and manifest.go for reuse and publication.
package generator
