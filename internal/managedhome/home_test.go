...
func fixture(t *testing.T) state.Identity {
	t.Helper()
	...
	identity.Home, err = state.HomePath(identity.HomeRoot, identity.Name, identity.ID)
	if err != nil {
		t.Fatal(err)
	}
	return identity
...
func TestPreexistingHomeIsNeverAdopted(t *testing.T) {
	...
	if _, err := Create(identity); err == nil {
...
func TestMismatchedAllocationCannotRemoveHome(t *testing.T) {
	...
	for _, change := range []func(*state.Identity){
		func(i *state.Identity) { i.Home = i.HomeRoot },
		func(i *state.Identity) { i.HomeRoot = i.Home },
		func(i *state.Identity) { i.Name = domain.VMName("other") },
		func(i *state.Identity) { i.ID = "abcdef012345" },
	} {
		changed := identity
		change(&changed)
		if err := Remove(changed); err == nil {
